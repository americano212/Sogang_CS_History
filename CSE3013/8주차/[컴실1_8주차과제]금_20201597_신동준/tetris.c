...
int modified_recommend(RecNode *root){
	...
	if(root->lv == BLOCK_NUM) return 0;
	for(int rot=0;rot<4;rot++){
		if (nextBlock[root->lv]==0 && rot>1){
			continue;
		}
		if (nextBlock[root->lv]==4 && rot>0){
			continue;
		}
		if (nextBlock[root->lv]==5 && rot>1){
			continue;
		}
		if (nextBlock[root->lv]==6 && rot>1){
			continue;
		}

		for(int x=-2;x<WIDTH+3;x++){
			int y= -1;
			if(CheckToMove(root->f,nextBlock[root->lv],rot,y,x)==0) continue;
			root->c[child_num] = (RecNode*)malloc(sizeof(RecNode));
			memcpy(root->c[child_num]->f,root->f,sizeof(root->f));

			root->c[child_num]->lv = root->lv+1;
			root->c[child_num]->score = 0;
			root->c[child_num]->side = 0;
			...
			root->c[child_num]->score = root->score+DeleteLine(root->c[child_num]->f);
			root->c[child_num]->score += plus_score;
			root->c[child_num]->score += modified_recommend(root->c[child_num]);
			root->c[child_num]->side += side_cnt;

			if(root->c[child_num]->score > max){
				max = root->c[child_num]->score;
				before_side = root->c[child_num]->side;
				if(root->c[child_num]->lv == 1){
					recommendX = x;recommendY = y;recommendR = rot;
					recommendblock = nextBlock[root->lv];
				}
			}
			else if (root->c[child_num]->score == max){
				if (root->c[child_num]->side > before_side){
					max = root->c[child_num]->score;
					before_side = root->c[child_num]->side;
					if(root->c[child_num]->lv == 1){
						recommendX = x;recommendY = y;recommendR = rot;
