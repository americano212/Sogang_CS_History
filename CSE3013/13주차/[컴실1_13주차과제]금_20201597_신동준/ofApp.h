...
class ofApp : public ofBaseApp {
	...
	public:
		...
		bool readFile();
		void freeMemory();
		bool DFS(int maze_col, int maze_row);
		bool BFS(int maze_col, int maze_row);
		void dfsdraw();
		void bfsdraw();
		int length = 50;
		int shift = -30;
		int HEIGHT;//미로의 높이
		int WIDTH;//미로의 너비
		vector<string> input;//텍스트 파일의 모든 정보를 담는 이차원 벡터이다.
		vector<vector<int>> visited;//방문여부를 저장할 벡터
